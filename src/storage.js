  ...
  try {
    ...
    try {
      ...
      if (hasTable(source, 'model_pricing')) {
        ...
        const value = (row, key, fallback = null) => columns.has(key) && row[key] != null ? row[key] : fallback;
        const upsertPricing = target.prepare(`INSERT INTO model_pricing (platform, model, input_usd_per_million, cached_input_usd_per_million, output_usd_per_million, reasoning_usd_per_million, cache_writes_usd_per_million, provider, pricing_unit, per_minute_usd, color, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(platform, model) DO UPDATE SET input_usd_per_million=excluded.input_usd_per_million, cached_input_usd_per_million=excluded.cached_input_usd_per_million, output_usd_per_million=excluded.output_usd_per_million, reasoning_usd_per_million=excluded.reasoning_usd_per_million, cache_writes_usd_per_million=excluded.cache_writes_usd_per_million, provider=excluded.provider, pricing_unit=excluded.pricing_unit, per_minute_usd=excluded.per_minute_usd, color=COALESCE(excluded.color, model_pricing.color), updated_at=excluded.updated_at`);
        const currentPricing = target.prepare('SELECT input_usd_per_million, cached_input_usd_per_million, output_usd_per_million, reasoning_usd_per_million, cache_writes_usd_per_million, per_minute_usd, color, updated_at FROM model_pricing WHERE platform=? AND model=?');
